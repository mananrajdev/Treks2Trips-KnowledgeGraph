...
df_struct.dropna(subset=['Common Names'], inplace=True)
df_struct.reset_index(inplace=True)
df_struct.drop('index', axis=1, inplace=True)
...
state_domain="http://treks2trips.org/state/"
sp_domain="http://treks2trips.org/species/"
      
# ----------------KG_GEN_STATES-----------------
for state in list(df_alltrails['state'].unique()):
    state_uri=URIRef(state_domain+state.lower().replace(" ","_"))
    my_kg.add((state_uri, RDF.type, SCHEMA.State))
    my_kg.add((state_uri, SCHEMA.name, Literal(state)))
    

# ----------------KG_GEN_NATIONAL_PARKS-----------------
for national_park in list(df_alltrails['national_park'].unique()):
    np_uri=URIRef(np_domain+national_park.lower().replace(" ","_").replace('"',''))
    my_kg.add((np_uri, RDF.type, SCHEMA.Park))
    my_kg.add((np_uri, SCHEMA.name, Literal(national_park)))

df_struct.drop_duplicates(subset=['Common Names', 'Category'])
...
# ----------------KG_GEN_SPECIES-----------------
for sp in list(df_struct['Common Names'].unique()):
    sp_uri_text=sp_domain+sp.lower().replace(" ","_").replace("`",'')
    if not sp_uri_text[-1].isalpha():
        sp_uri_text=sp_uri_text[:-1]
    sp_uri=URIRef(sp_uri_text)
    my_kg.add((sp_uri, RDF.type, MYNS.species))
    my_kg.add((sp_uri, SCHEMA.name, Literal(sp)))
    my_kg.add((sp_uri, SCHEMA.category, Literal(list(df_struct[df_struct['Common Names']==sp]['Category'])[0].strip())))
...
for i in range(len(df_alltrails)):
    ...
    state_uri=URIRef(state_domain+df_alltrails.loc[i,"state"].lower().replace(" ","_"))

    my_kg.add((node_uri, RDF.type, MYNS['Trail'])) 
    my_kg.add((node_uri, SCHEMA.name, Literal(df_alltrails.loc[i,"name"])))
    

    my_kg.add((node_uri, SCHEMA.inNationalPark, np_uri))
    
    
    
    my_kg.add((node_uri, MYNS.inState, state_uri))
    ...
    my_kg.add((node_uri, MYNS.routeType, Literal(df_alltrails.loc[i,"route_type"])))
    my_kg.add((node_uri, MYNS.dogStatus, Literal(df_alltrails.loc[i,"dog_status"])))
    my_kg.add((node_uri, MYNS.kidStatus, Literal(df_alltrails.loc[i,"kid_status"])))
    my_kg.add((node_uri, MYNS.bikeStatus, Literal(df_alltrails.loc[i,"bike_status"])))
...
for i in range(len(df_other3)):
    ...
    state_uri=URIRef(state_domain+df_other3.loc[i,"state"].lower().replace(" ","_"))
    my_kg.add((node_uri, MYNS.inState, state_uri))
    
    
    
    if not pd.isna(df_other3.loc[i,"Scenic Drive"]):
        my_kg.add((node_uri, MYNS.scenicDrive, Literal(df_other3.loc[i,"Scenic Drive"])))
    
    if not pd.isna(df_other3.loc[i,"Best Time to Go"]):
        my_kg.add((node_uri, MYNS.bestTime, Literal(df_other3.loc[i,"Best Time to Go"])))
    ...
    if not pd.isna(df_other3.loc[i,"highest_peak"]):
        my_kg.add((node_uri, MYNS.highestPeak, Literal(df_other3.loc[i,"highest_peak"])))
    ...
    if not pd.isna(df_other3.loc[i,"hotel_name"]):
        ...
        for h in range(len(hotels_list)):
            ...
            my_kg.add((node_uri, MYNS.hasHotel, hotel_uri))
...
for i in range(len(df_struct)):
    ...
    sp_uri_text=sp_domain+df_struct.loc[i,'Common Names'].lower().replace(" ","_").replace("`",'')
    if not sp_uri_text[-1].isalpha():
        sp_uri_text=sp_uri_text[:-1]
    sp_uri=URIRef(sp_uri_text)
    my_kg.add((node_uri, MYNS.hasSpecies, sp_uri))
...
# ----------------SAVE FILE-----------------
my_kg.serialize('../Datasets/KG_final_triples_animals.ttl', format="turtle")
